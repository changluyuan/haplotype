...
open (REFERENCEPOSITION, "<$ARGV[1].split.plus.$ARGV[3].CGMAP");
open (OUTPUT, ">$ARGV[2]");
...
#open O,"|/gpfs/bin/samtools-1.3.1/samtools view -Sbh -F 1024 > $ARGV[0].plus.tag.bam";
#while(<F>){
 #   if(/^\@/){
  #      print O $_;
   # }else{
...
#}
#close O;
#`/gpfs/bin/samtools-1.3.1/samtools index $ARGV[0].plus.tag.bam`;
